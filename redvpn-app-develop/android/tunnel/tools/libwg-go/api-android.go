...
func (l AndroidLogger) Write(p []byte) (int, error) {
	C.__android_log_write(l.level, C.CString("RedVPN/GoBackend/"+l.interfaceName), C.CString(string(p)))
	return len(p), nil
}
	...
	go func() {
		...
		for {
			...
			case <-signals:
				...
				buf[n] = 0
				C.__android_log_write(C.ANDROID_LOG_ERROR, C.CString("RedVPN/GoBackend/Stacktrace"), (*C.char)(unsafe.Pointer(&buf[0])))
			}
		}
	}()
}
